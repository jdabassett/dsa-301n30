...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let returnArray = arr.map(item => (item.baseStat>minBaseStat?item.stat.name:null));
  return returnArray.filter(item => item!==null);
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(item => !item.hasOwnProperty('children'))
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let returnArray = arr.filter(item => typeof(item)==='number');
  return returnArray.map(item => ((item%2===0)?'even':'odd'));
};
