...
const validatePin = (pin) => {
  // Solution code here...
  let regexPass = /^[0-9]{4}$/g;
  return regexPass.test(pin);
};
...
const validateWord = (word) => {
  // Solution code here...
  let regexPass = /^\D{5,10}$/g;
  return regexPass.test(word);
};
...
const hasNumber = (string) => {
  // Solution code here...
  let regexTrue = /[a-z]{1,}[0-9]{1,}/gi;
  return regexTrue.test(string);
};
...
const validateEmail = (email) => {
  // Solution code here...
  let regexPass = /^[A-Za-z0-9]+\.?[A-Za-z0-9]+@[A-Za-z]+\.(net|com|org)$/;
  // let regexFail = /[\S+.\S+.\S+]@/g;
  // && regexFail.test(email)
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  // let numberCount = phoneNumber.split("").reduce((acc, b) => {
  //   parseInt(b) >= 0 ? (acc += 1) : acc;
  //   return acc;
  // }, 0);
  let regexPass = /(\([0-9]{3}\)\s?|^[0-9]{3}[\s-_]?)[0-9]{3}[\s-_]?[0-9]{4}$/g;
  return regexPass.test(phoneNumber);
};
