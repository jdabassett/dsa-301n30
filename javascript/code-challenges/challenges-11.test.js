...
function transformToLis(obj){
  // Solution code here...
  let returnArray = [];
  Object.entries(obj).forEach(item => returnArray.push(`<li>${item[0]}: ${item[1]}</li>`));
  return returnArray;
}
...
//   // Solution code here...
//   let returnArray = [];
//   input.forEach(item0 => item0.forEach(item1=> (item1===target)?returnArray.push(item1):null));
...
const count = (target, input) => {
  // Solution code here...
  let returnCount = 0;
  input.forEach(nestedArray => {returnCount+=nestedArray.reduce((acc,b)=>{b===target?acc+=1:acc; return acc;},0);});
  return returnCount;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc,b)=>{return acc+=(b.reduce((acc,c)=>{return acc+=c;},0));},0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(nestedArray => nestedArray.filter(number=> (number%5===0) && (typeof number==='number')).map(numberF=> {return 2**numberF}));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((acc,b)=>{(acc.height<b.height)?acc=b:acc; return acc;},{height:0}).name;
};
