...
const screenForNames = (arr) => {
  // Solution code here...
  // let returnArray = [];
  let regex = /^(Mr.\s|Mrs.\s|Ms.\s|Dr.\s)[A-Za-z]/g;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let lukeObject = arr.filter(item => item.name==="Luke Skywalker")[0];
  return arr.filter(item => parseInt(lukeObject.mass) < parseInt(item.mass)).reduce((acc,b)=>{acc.push(b.name);return acc;},[]).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let propertyParsed = `${property}`;
  if (typeof arr[0][propertyParsed]!=='number'){return arr.sort((a,b)=> a[propertyParsed].localeCompare(b[propertyParsed]));
  } else {return arr.sort((a,b)=> a[propertyParsed]-b[propertyParsed]);};
};
...
const helperFunction = (board, arr) => {
  if (board[arr[0][0]][arr[0][1]]==="" || board[arr[1][0]][arr[1][1]]==="" || board[arr[2][0]][arr[2][1]]==="") {
    return false;
  } else if (board[arr[0][0]][arr[0][1]]===board[arr[1][0]][arr[1][1]] && board[arr[0][0]][arr[0][1]]===board[arr[2][0]][arr[2][1]]){
    return true;
  }
  return false;
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i=0; i<indicesArray.length; i++){
    returnBool = helperFunction(board,indicesArray[i]);
