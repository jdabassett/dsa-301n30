...
const maxInArray = (arr) => {
  // Solution code here...
  return arr.reduce((acc,b)=>{(acc<b)?acc=b:acc; return acc;},0);
};
...
const hasChildrenValues = (arr, character) => {
  ...
  // Solution code here...
  let obj = arr.filter(item=>item.name===character)[0];
  Object.values(obj).forEach(item=>(typeof(item)==='object'?returnBool=true:null));
  return returnBool;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.entries(obj).forEach(pair=>{
    if(pair[0]==='children'){returnBool=true;}
  });
  return returnBool;
};
/* ------------------------------------------------------------------------------------------------
...
const returnNumberFromObject = (obj) => {
  let objectTotal = 0;
  let newObj = obj;
  // delete newObj.house;
  let array = Object.keys(newObj);
  array.forEach(key=>{
    if(typeof(obj[key])==='string' && key==='name'){
      objectTotal+=1;
    } else if (typeof(obj[key])==='string' && key==='spouse') {
      objectTotal+=1;
    } else if (typeof(obj[key])==='object' && key==='children') {
      objectTotal+=obj[key].length;
    }
  });
...
const totalCharacters = (arr) => {
  // Solution code here...
  let arrayTotal = 0;
  arr.forEach(item=> arrayTotal+=returnNumberFromObject(item));
  return arrayTotal;
};
...
const houseSize = (arr) => {
  const sizes = [];
  // Solution code here...
  arr.forEach(item=> sizes.push({house:item.house,members:returnNumberFromObject(item)}));
  return sizes;
};
...
const returnLivingNumberFromObject = (obj,arr) => {
  let objectTotal = 0;
  let newObj = obj;
  delete newObj.house;
  let array = Object.keys(newObj);
  array.forEach(key=>{
    if(typeof(obj[key])==='string' && key==='name' && !arr.includes(obj[key])){
      objectTotal+=1;
    } else if (typeof(obj[key])==='string' && key==='spouse' && !arr.includes(obj[key])) {
      objectTotal+=1;
    } else if (typeof(obj[key])==='object' && key==='children') {
      obj[key].forEach(child=> (!arr.includes(child)?objectTotal+=1:null));
    }
  });
...
const houseSurvivors = (arr) => {
  const survivors = [];
  // Solution code here...
  arr.forEach(item=> survivors.push({house:item.house,members:returnLivingNumberFromObject(item,deceasedSpouses)}));
  return survivors;
};
