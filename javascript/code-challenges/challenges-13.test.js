...
// Solution code here...
  let longestString = arr.reduce((acc,b)=>{acc.length<b.length?acc=b:acc; return acc;},'');
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.reduce((acc,b)=>{acc.push(b.charAt(0)); return acc;},[]);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  // return arr.map(item=> item.split("(").join("").split(")").join("").split(" ").join("").split("-").join(""));
  let regex = /[()\s-]/g;
...
const onlyOddChars = (str) => {
  // Solution code here...
  let newArr = [];
  str.split("").forEach((item,idx)=> (idx%2===1)?newArr.push(item):null);
  return newArr.join("");
};
...
const allHappy = (arr) => {
  ...
  // let regex = /:\)/g;
  let boolArr = [];
  arr.forEach(item => boolArr.push(item.includes(":)")));
  let bool = boolArr.filter(item => item===false).length===0;
  return bool;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let boolArr = [];
  arr.forEach(item => boolArr.push(item.includes(target)));
  let bool = boolArr.filter(item => item===false).length===0;
  return bool;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(students => students.filter(student => (!student.includes('Brook'))));
};
