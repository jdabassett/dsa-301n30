...
const stepActions = (recipe) => {
  ...
  recipe
    .steps
    .map(item => result.push(item.split(" ")[0]));
...
const totalSumCSV = (str) => {
  let total = 0;
  // Solution code here...
  str.split(",").map(item=> total+=parseInt(item));
  return total;
};
...
const removeVowels = (str) => {
  // Solution code here...
  return str.split("").filter(item => {return !['a','e','i','o','u'].includes(item)}).join("");
};
...
const extractVowels = (str) => {
  ...
  str.split("").map(item => ['a','e','i','o','u'].includes(item)? arrayV.push(item): arrayWOV.push(item));

  return [arrayWOV.join(""), arrayV.sort((a,b)=> a.localeCompare(b)).join("")];
};
